...
CREATE TABLE sales.loyalty_cards (
    ...
    tier          VARCHAR(10)  NOT NULL
                               CHECK (tier IN ('Bronze', 'Silver', 'Gold')),  -- valid tiers only
    join_date     DATE         NOT NULL,                          -- required

    CONSTRAINT fk_loyalty_customer
        FOREIGN KEY (customer_id)
        REFERENCES sales.customers (customer_id)
        ON DELETE CASCADE                                         -- delete card if customer deleted
);
...
-- Step 2: Add the constraint via ALTER TABLE (no recreation)
...
SELECT
    ...
    CASE
        ...
        WHEN shipped_date - order_date <= 2                             THEN 'Fast'
        WHEN shipped_date - order_date BETWEEN 3 AND 5                 THEN 'Normal'
        ELSE                                                                 'Delayed'
